...
Watch the Docker logs immediately** **using** `docker-compose logs batch-datamodeling-app`.

**Insert the necessary data** into `bank_a_transactions`:


bash

`docker exec -it postgres-db psql -U youruser -d datamodelingdb -c "TRUNCATE TABLE bank_a_transactions;"`

`docker exec -it postgres-db psql -U youruser -d datamodelingdb -c "TRUNCATE TABLE FACT_TRANSACTIONS;"`

```
docker exec -it postgres-db psql -U youruser -d datamodelingdb -c "INSERT INTO bank_a_transactions (bank_specific_account_id, transaction_date, amount, description, location_code) VALUES ('ACC123', '2023-01-01 10:00:00', '150.00', 'Coffee Shop', 'L1'), ('ACC124', '2023-01-01 11:00:00', '20.00', 'Gas Station', 'L2');"
```


**IMMEDIATELY Verify the count**

bash

```
docker exec -it postgres-db psql -U postgres -d datamodelingdb -c "SELECT count(*) FROM bank_a_transactions;"
```


**Trigger the Data Modelling Job (Service 2) via Postman** **immediately after confirming the count:**

* **URL:** `http://localhost:8080/api/v1/ingestion/start/BANK_A`
* **Method:** `POST`


The job will now read the 2 rows from the table, send them to the processor, and write them to the fact table. The E2E pipeline is now fully verified.
...
CREATE TABLE IF NOT EXISTS FACT_TRANSACTIONS (
    ...
    description_standard VARCHAR(255),
    location_code VARCHAR(255),
    transaction_type VARCHAR(255),
    is_valid BOOLEAN
);

CREATE TABLE IF NOT EXISTS customer_mdm_entity (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    bank_specific_account_id VARCHAR(255),
    unified_customer_id VARCHAR(255)
);

-- Insert mapping for the sample data we generated previously:
INSERT INTO customer_mdm_entity (bank_specific_account_id, unified_customer_id) VALUES 
('ACC123', 'U_CUST_GLOBAL_A45'),
('ACC124', 'U_CUST_GLOBAL_B89');
```
